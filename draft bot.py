    ...
    def __init__(self, channel_id):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                child.custom_id = f"manual_start_{channel_id}"

    @discord.ui.button(label="Manual Start", style=discord.ButtonStyle.danger, custom_id="manual_start")
...
class MiddleManButton(discord.ui.View):
    def __init__(self, channel_id):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                child.custom_id = f"middleman_confirm_{channel_id}"

    @discord.ui.button(label="I'm the Middle Man", style=discord.ButtonStyle.primary, custom_id="middleman_confirm")
...
class CashAppSubmitView(discord.ui.View):
    def __init__(self, channel_id):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                child.custom_id = f"submit_cashapp_{channel_id}"

    @discord.ui.button(label="Submit Cash App Username", style=discord.ButtonStyle.green, custom_id="submit_cashapp")
...
class PaymentControlView(discord.ui.View):
    def __init__(self, channel_id, *, mode: str = "initial"):
        super().__init__(timeout=None)
        self.channel_id = channel_id
        self.mode = mode  # "initial", "double", or "fullpot"
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                child.custom_id = f"payment_manual_{channel_id}"

    @discord.ui.button(label="Start Draft Manually", style=discord.ButtonStyle.red, custom_id="payment_manual")
    async def manual_start(self, interaction: discord.Interaction, button: discord.ui.Button):
        data = load_drafts()
        draft = data.get(str(self.channel_id))
        ...
        if self.mode == "double":
            draft.setdefault("double", {})["state"] = "complete"
            save_drafts(data)
            roles = team_role_mentions(interaction.guild, draft)
            await interaction.channel.send(f"{roles} Double payment confirmed. You may play now.")
            await send_fullpot_prompt(interaction.channel)
            return

        if self.mode == "fullpot":
            draft.setdefault("fullpot", {})["state"] = "complete"
            save_drafts(data)
            roles = team_role_mentions(interaction.guild, draft)
            await interaction.channel.send(f"{roles} Payments confirmed. You may play now.")
            await send_even_close_prompt(interaction.channel)
            return
...
class DoublePromptView(discord.ui.View):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                if child.label == "Double":
                    child.custom_id = f"double_vote_{channel_id}"
                else:
                    child.custom_id = f"double_close_{channel_id}"

    @discord.ui.button(label="Double", style=discord.ButtonStyle.green, custom_id="double_vote")
    ...
    @discord.ui.button(label="Close Draft", style=discord.ButtonStyle.red, custom_id="double_close")
...
class FullPotPromptView(discord.ui.View):
    ...
    def __init__(self, channel_id):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                if child.label == "Full Pot":
                    child.custom_id = f"fullpot_vote_{channel_id}"
                elif child.label == "Went Even":
                    child.custom_id = f"fullpot_even_{channel_id}"
                else:
                    child.custom_id = f"fullpot_close_{channel_id}"

    @discord.ui.button(label="Full Pot", style=discord.ButtonStyle.green, custom_id="fullpot_vote")
    ...
    @discord.ui.button(label="Went Even", style=discord.ButtonStyle.gray, custom_id="fullpot_even")
    ...
    @discord.ui.button(label="Close Draft", style=discord.ButtonStyle.red, custom_id="fullpot_close")
    async def close(self, interaction: discord.Interaction, button: discord.ui.Button):
        disable_all_buttons(self)
        if self.message:
            await self.message.edit(view=self)
...
class PostFullPotView(discord.ui.View):
    """Prompt shown after a full pot rematch to go even or close the draft."""
    ...
    def __init__(self, channel_id):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                if child.label == "Went Even":
                    child.custom_id = f"postfull_even_{channel_id}"
                else:
                    child.custom_id = f"postfull_close_{channel_id}"

    @discord.ui.button(label="Went Even", style=discord.ButtonStyle.gray, custom_id="postfull_even")
    async def even(self, interaction: discord.Interaction, button: discord.ui.Button):
        data = load_drafts()
        draft = data.get(str(self.channel_id))
        if not draft:
    ...
    @discord.ui.button(label="Close Draft", style=discord.ButtonStyle.red, custom_id="postfull_close")
    async def close(self, interaction: discord.Interaction, button: discord.ui.Button):
        disable_all_buttons(self)
        if self.message:
            await self.message.edit(view=self)
...
class LoserButtonView(discord.ui.View):
    def __init__(self, channel_id):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                if child.label == "Team 1":
                    child.custom_id = f"loser_team1_{channel_id}"
                elif child.label == "Team 2":
                    child.custom_id = f"loser_team2_{channel_id}"
    ...
    @discord.ui.button(label="Team 1", style=discord.ButtonStyle.blurple, custom_id="loser_team1")
    async def team1(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._process_vote(interaction, "team1")

    @discord.ui.button(label="Team 2", style=discord.ButtonStyle.red, custom_id="loser_team2")
...
async def send_even_close_prompt(channel):
    """After a full pot match begins, offer captains options to go even or close."""
    data = load_drafts()
    draft = data.get(str(channel.id))
    if not draft:
        return
    ...
    embed = discord.Embed(
        title="Play Again?",
        description=(
            f"{cap_mentions} can click **Went Even** to restart the series or **Close Draft** to finish."
        ),
        color=discord.Color.dark_gold(),
    )
    view = PostFullPotView(channel.id)
    msg = await channel.send(embed=embed, view=view)
    view.message = msg

    draft.setdefault("fullpot", {})["post_message_id"] = msg.id
    bot.add_view(view, message_id=msg.id)
    save_drafts(data)
...
class PickButton(discord.ui.Button):
    def __init__(self, player, channel_id):
        # Assign a stable custom_id so the view can be reattached after a restart
        custom = f"pick_{channel_id}_{player.id}"
        super().__init__(label=player.display_name, style=discord.ButtonStyle.primary, custom_id=custom)
        self.player = player
...
async def begin_cashapp_collection(guild, channel):
    ...
    msg = await channel.send(embed=embed, view=view)
    draft["submit_tags_msg_id"] = msg.id
    bot.add_view(view, message_id=msg.id)

    manual_msg = await channel.send(view=manual_view)
    draft["manual_start_msg_id"] = manual_msg.id
    bot.add_view(manual_view, message_id=manual_msg.id)
    save_drafts(data)
...
async def send_payment_instructions(channel, players=None, *, mode: str = "initial"):
    ...
    # Manual Start Button
    view = PaymentControlView(channel.id, mode=mode)
    msg = await channel.send(content=player_mentions, embed=embed, view=view)
    draft["payment_message_id"] = msg.id
    bot.add_view(view, message_id=msg.id)
    save_drafts(data)
...
async def send_pick_options(channel):
    ...
    msg = await channel.send(f"{pick_captain.mention}, it's your turn to pick:", view=view)
    bot.add_view(view, message_id=msg.id)
...
# NEW: Allow role 1374569702801670144 to view the channel
    ...
    bot.add_view(view, message_id=queue_message.id)
    save_drafts(data)
                                ...
                                if sender_norm == tag_norm:
                                    ...
                                    if all_paid and channel:
                                        ...
                                        if double_state == "collecting":
                                            ...
                                            save_drafts(data)
                                            asyncio.run_coroutine_threadsafe(send_even_close_prompt(channel), bot.loop)
                                        else:
...
async def on_ready():
    await tree.sync()
    data = load_drafts()
    for cid, draft in data.items():
        channel_id = int(cid)
        if msg_id := draft.get("queue_message_id"):
            max_players = int(draft.get("team_size", "3v3")[0]) * 2
            bot.add_view(DraftQueueView(channel_id, max_players), message_id=msg_id)
        if msg_id := draft.get("middleman_select_msg_id"):
            bot.add_view(MiddleManButton(channel_id), message_id=msg_id)
        if msg_id := draft.get("submit_tags_msg_id"):
            bot.add_view(CashAppSubmitView(channel_id), message_id=msg_id)
        if msg_id := draft.get("manual_start_msg_id"):
            bot.add_view(ManualStartView(channel_id), message_id=msg_id)
        if msg_id := draft.get("payment_message_id"):
            mode = draft.get("collection_mode", "initial")
            bot.add_view(PaymentControlView(channel_id, mode=mode), message_id=msg_id)
        if msg_id := draft.get("double", {}).get("message_id"):
            bot.add_view(DoublePromptView(channel_id), message_id=msg_id)
        if msg_id := draft.get("double", {}).get("loser_message_id"):
            bot.add_view(LoserButtonView(channel_id), message_id=msg_id)
        if msg_id := draft.get("fullpot", {}).get("message_id"):
            bot.add_view(FullPotPromptView(channel_id), message_id=msg_id)
        if msg_id := draft.get("fullpot", {}).get("post_message_id"):
            bot.add_view(PostFullPotView(channel_id), message_id=msg_id)
        if msg_id := draft.get("fullpot", {}).get("loser_message_id"):
            bot.add_view(LoserButtonView(channel_id), message_id=msg_id)
